  ...
  return (
    <div>
      <PageHeader title="Payments" subtitle="Track client payments" action={
        <button onClick={() => { setEditing(null); setForm({ client: '', clientName: '', project: '', totalAmount: '', advancePaid: '', dueDate: '', notes: '' }); setShowModal(true); }} className="btn-primary flex items-center gap-2">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-secondary-100 dark:border-secondary-700">
                  ...
                  <th className="text-left py-3 px-2 font-medium text-secondary-500">Total</th>
                  <th className="text-left py-3 px-2 font-medium text-secondary-500">Paid</th>
                  <th className="text-left py-3 px-2 font-medium text-secondary-500">Remaining</th>
                  <th className="text-left py-3 px-2 font-medium text-secondary-500">Due Date</th>
              ...
              <tbody>
                {payments.map((p) => (
                  <tr key={p._id} className="border-b border-secondary-50 dark:border-secondary-700/50 hover:bg-secondary-50 dark:hover:bg-secondary-700/30">
                    <td className="py-3 px-2 font-medium">{p.clientName}</td>
                    <td className="py-3 px-2">{formatCurrency(p.totalAmount)}</td>
                    <td className="py-3 px-2 text-green-600">{formatCurrency(p.advancePaid)}</td>
                    <td className="py-3 px-2 text-red-500">{formatCurrency(p.remainingAmount)}</td>
                    <td className="py-3 px-2 text-secondary-500">{formatDate(p.dueDate)}</td>
